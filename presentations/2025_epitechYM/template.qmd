...
- **Code Hoarding Disorder (CHD)**. It's characterized by the persistent difficulty in discarding or deleting unused code files, despite their lack of current value to the project. It is also called the `analysis_v2_final_FINAL_realFinal_USE_THIS_ONE.R` disease. Frequently co-occuring with **Version Control Avoidance Syndrome (VCAS)** or **I-push-everything-to-the-main-branch-syndrome-because-YOLO** (too long acronym).
...
(maybe he already told me that before, but sometime he speaks too fast for me to understand everything)
...
**A task is a function, a function is a task**
...
- We need to compute the time since vaccination statistics (median, IQR) in several groups. 

- We write a R code to do that:

```r
compute_time_since_vaccination <- function(data, some_groups) {
...
- In R we use `expect_xxx()` functions:
  ...
  - `expect_error()` to check that the function returns an error
  - `expect_message()` to check that the function returns a message
  - ...
:::
...
## Unit tests as an surveillance system
...
**You won't be scared anymore of breaking something when implementing a new feature in your code !**
...
In a dev perspective, it helps to keep track of the tasks to be done, and to prioritize them.
...
On culbabilise pas car chacun fait comme il peut en fonction de son temps, de ses connaissances 

Résister à la tentation du quick and diry aujourdrhui et promis je corrigerai plus tard
...
### Examples can be copied and pasted from documentation for otther purposes

### Montrer le repo GH avec les branches et les issues